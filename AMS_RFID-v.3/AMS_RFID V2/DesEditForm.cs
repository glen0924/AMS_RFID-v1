...
{
    public partial class DesEditForm : Form
    {
        public DesEditForm()
        {
            InitializeComponent();

            DesComboSelect.SelectedIndexChanged += DesComboSelect_SelectedIndexChanged;

        }

        private void DesEditForm_Load(object sender, EventArgs e)
        {
            Mysqlloop();
            DesTxtbx.Enabled = false;
        ...
        void Mysqlloop()
        {
            ...
            {
                string querry = "SELECT * FROM designation";
                mysql.Open();
                ...
                {
                    DesComboSelect.Items.Add(readd.GetString("designationName"));
                }
                mysql.Close();
            }
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                using (MySqlConnection mysql = new MySqlConnection(MyDsql))
                {
                    string querry = "UPDATE designation SET designationName= @designationName WHERE designationName= @desName";
                    ...
                    cmdoy.Parameters.AddWithValue("@designationName", DesTxtbx.Text);
                    cmdoy.Parameters.AddWithValue("@desName", DesComboSelect.Text);
                    MySqlDataReader readd = cmdoy.ExecuteReader();

                    mysql.Close();
                }
                MessageBox.Show("Successfully Saved Data", "Message");
                DesComboSelect.Items.Clear();
                Mysqlloop();
        ...
        private void DesComboSelect_SelectedIndexChanged(object sender, EventArgs e)
        {
            DesTxtbx.Text = DesComboSelect.Text;
        }
