...
namespace AMS_RFID_V2
{
    public partial class EmployeeAttendance : Form
    {
        ...
        {
            // TODO: This line of code loads data into the 'ams_rfidDataSetEmployee.employees' table. You can move, or remove it, as needed.
        ...
        private void EmployeeAttendance_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        private void AmPm_Tick(object sender, EventArgs e)
        {
            ...
            {
                Greetings.ForeColor = Color.DarkOrange;
        ...
        {
            if (Application.OpenForms.OfType<MainPage>().Count() == 1)
            {
                Application.OpenForms.OfType<MainPage>().First().BringToFront();
                Application.OpenForms.OfType<MainPage>().First().WindowState = FormWindowState.Normal;
            }
            else
            {
            ...
            try
            {
              ...
              new Size(20, 20));
                ...
                foreach (MCvAvgComp f in facesDetected[0])
                {
                    ...
                    emp.Text = facesDetected[0].Length.ToString();
                }
                t = 0;
        ...
        private void sql()
        {
            ...
            using (MySqlConnection iconnect = new MySqlConnection(MyDsql))
            {
                ...
                using (MySqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        using (MySqlConnection iconnectg = new MySqlConnection("Datasource=localhost;port=3306;username=AMS_RFID;password=@M$_Rf1d2O22; database = ams_rfid; Max Pool Size=1000;Convert Zero Datetime=True"))
                        {
        ...
        {
            using (MySqlConnection iconnect = new MySqlConnection("Datasource=localhost;port=3306;username=AMS_RFID;password=@M$_Rf1d2O22; database = ams_rfid; Max Pool Size=1000;Convert Zero Datetime=True"))
            {
                        ...
                        using (MySqlDataReader reader = command.ExecuteReader())
                        {
                            ...
                            if (reader.Read())
                            {
                                ...
                                if ((now < startam))
                                {
                                    ...
                                    {
                                        string selectquerry1 = "SELECT `EmployeeRfid`, `Date` FROM `attendance` WHERE EmployeeRfid = '" + this.btnScan.Text + "' AND Date= '" + date + "' AND Am_In IS NULL ";
                                ...
                                else if ((now > startam1) && (now <= endam1))
                                {
                                    ...
                                    using (MySqlConnection iconnectSelect1 = new MySqlConnection(MyDsql))
                                    {
                                        ...
                                        string selectquerry11 = "SELECT `EmployeeRfid`, `Date` FROM `attendance` WHERE EmployeeRfid = '" + this.btnScan.Text + "' AND `attendance`.`Date` = '" + date + "' AND Am_In IS NULL";
                                ...
                                //{
                                //    AutoClosingMessageBox.Show("Log Denied.Consult your Admin", "Error", 2000);
                                //    btnScan.Clear();
                                //}
                                    ...
                                    using (MySqlConnection iconnectSelect = new MySqlConnection(MyDsql))
                                    {
                                        ...
                                        if (readerSelect.Read())
                                        {
                                            using (MySqlConnection pmsearchin = new MySqlConnection(MyDsql))
                                            {
                                                ...
                                                string insertquerry1 = "UPDATE `attendance` SET `Am_Out` = current_timestamp() WHERE `attendance`.`Date` = '" + date + "' AND `attendance`.`EmployeeRfid` = '" + this.btnScan.Text + "'";
                                        ...
                                        {
                                            //AutoClosingMessageBox.Show("Already Log", "Information: 12:00 - 12:59", 2000);
                                            //btnScan.Clear();
                                            //AutoClosingMessageBox.Show("Already Log 1", "Warning", 1500);
                                            ...
                                            using (MySqlConnection pmsearchinX = new MySqlConnection(MyDsql))
                                            {
                                                ...
                                                // string selectquerry1q = "SELECT `EmployeeRfid`, `Date` ,`Am_Out` FROM `attendance` WHERE EmployeeRfid = @EmployeeRfid AND Date = '" + date + "'  AND Am_Out <  '" + dt2 + "'   AND Am_Out IS NOT NULL";
                                                    ...
                                                    {//using (MySqlConnection msql = new MySqlConnection(MyDsql))
                                                        {
                                                            ...
                                                            string insertquerry1 = "UPDATE `attendance` SET `Pm_In` = current_timestamp() WHERE `attendance`.`Date` = '" + date + "' AND `attendance`.`EmployeeRfid` = '" + this.btnScan.Text + "'";
                                ...
                                {
                                    //AutoClosingMessageBox.Show("Log Denied. Time insufficient", "Error", 2000);
                                    //btnScan.Clear();
                                    string date = DateTime.Now.ToString("yyyy-MM-dd");
                                ...
                                else if (dtime.Hour == 17)
                                {
                                    ...
                                    using (MySqlConnection iconnectSelectend = new MySqlConnection(MyDsql))
                                    {
                                        ...
                                        if (readerSelectend.Read())
                                        {
                                            ...
                                            string insertquerry1 = "UPDATE `attendance` SET `Pm_Out` = '" + dtime1 + "' WHERE `attendance`.`Date` = '" + date + "' AND `attendance`.`EmployeeRfid`= '" + this.btnScan.Text + "'";
